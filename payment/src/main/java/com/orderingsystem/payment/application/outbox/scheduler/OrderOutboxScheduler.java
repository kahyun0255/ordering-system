...
public class OrderOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderOutbox>> outboxMessagesResponse = orderOutboxHelper.getOrderOutboxMessageByOutboxStatus(OutboxStatus.STARTED);

        if (outboxMessagesResponse.isPresent() && !outboxMessagesResponse.get().isEmpty()){
            List<OrderOutbox> outboxMessages = outboxMessagesResponse.get();

            log.info("{}개의 Payment OrderOutbox Message를 수신했으며, 해당 ID {}들을 메시지 버스로 전송합니다.",
            ...
            log.info("{}개의 Payment OrderOutbox Message를 메시지 버스로 전송했습니다.", outboxMessages.size());
        }
    }
...
//        orderOutbox.updateProcessedAt(ZonedDateTime.now());
        orderOutboxHelper.save(orderOutbox);
        log.info("Payment OrderOutbox 메시지의 OutboxStatus를 {}로 업데이트 했습니다.", outboxStatus.name());
    }
}
