...
public class RestaurantValidateOrderService {
    ...
    public OrderApprovalEvent validateOrder(RestaurantInfo restaurant, List<String> failureMessages) {
        log.info("주문 검증 시작. Order Id : {}", restaurant.getOrderDetail().getOrderId());
        restaurant.validateOrder(failureMessages);

        if (failureMessages.isEmpty()) {
            return restaurant.approveOrder(failureMessages);
        } else {
            return restaurant.rejectOrder(failureMessages);
        }
    }
}
