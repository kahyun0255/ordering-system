...
public class RestaurantValidationService {
    ...
    public void validate(Restaurant restaurant, RestaurantValidationApplicationRequest restaurantValidationRequest) {
        ...
        Map<UUID, Item> itemMap = mergeDuplicateItems(restaurantValidationRequest.getItems());
        ...
        validateRequestedQuantities(restaurantValidationRequest, restaurantValidationRequest.getSagaId());
        ...
        validateStockAvailability(products, restaurantValidationRequest);
    }

    private Map<UUID, Item> mergeDuplicateItems(List<Item> items) {
        Map<UUID, Item> result = new HashMap<>();
        for (Item item : items) {
            result.merge(
                    item.getProductId(),
                    item,
                    (oldV, newV) -> Item.builder()
    ...
    private void validateRestaurant(Restaurant restaurant, UUID sagaId) {
        if (!restaurant.getStatus().equals(RestaurantStatus.ACTIVE)) {
            log.info("레스토랑이 주문 가능한 상태가 아닙니다. Restaurant Id : {}, Saga Id : {}", restaurant.getRestaurantId(), sagaId);
            throw new IllegalArgumentException("레스토랑이 주문 가능한 상태가 아닙니다.");
        }
    }

    private void validateRequestedQuantities(RestaurantValidationApplicationRequest request, UUID sagaId) {
        request.getItems().forEach(item -> {
            if (item.getQuantity() <= 0) {
                log.info("상품 수량이 유효하지 않습니다. Product Id : {}, Quantity : {}, Saga Id : {}",
                        item.getProductId(), item.getQuantity(), sagaId);
    ...
    private void validateProductsExist(UUID sagaId, List<Product> products, List<UUID> productIds) {
        ...
        if (!missing.isEmpty()) {
            log.info("주문에 존재하지 않는 상품이 포함되어있습니다. Product Id : {}, Saga Id : {}", missing, sagaId);
    ...
    private void validateRestaurantSellsProducts(Restaurant restaurant,
        ...
        if (!notSellable.isEmpty()) {
            log.info("해당 레스토랑이 판매하지 않는 상품이 있습니다. Restaurant Id : {}, Product Id : {}, Saga Id : {}",
    ...
    private void validateProductAvailabilityAndPrice(RestaurantValidationApplicationRequest restaurantValidationRequest,
        ...
        for (Product product : products) {
            if (!product.isAvailable()) {
                log.info("주문에 판매 불가능한 상품이 포함되어 있습니다. Product Id : {}, Saga Id : {}", product.getProductId(),
            ...
            if (product.getPrice().getAmount().compareTo(item.getPrice()) != 0) {
                log.info("주문을 요청한 상품의 가격과 판매 가격이 일치하지 않습니다. Product Id : {}, 주문 가격 : {}, 판매 가격 : {}, Saga Id : {}",
    ...
    private void validateTotalPrice(RestaurantValidationApplicationRequest restaurantValidationRequest,
        ...
        if (restaurantValidationRequest.getTotalPrice() == null
            ...
            log.info("총 주문 금액이 일치하지 않습니다. 요청 합계 : {}, 계산 합계 : {}, Saga Id : {}",
    ...
    private void validateStockAvailability(List<Product> products, RestaurantValidationApplicationRequest request) {
        ...
        request.getItems().forEach(item -> {
            ...
            if (product.getQuantity() < item.getQuantity()) {
                log.info("상품 재고가 부족합니다. Product Id : {}, 요청 수량 : {}, 현재 재고 : {}",
